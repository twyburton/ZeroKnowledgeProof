package uk.ac.ncl.burton.twy.proofOfConceptTesting;
...
public class ProofOfKnowledgeTesting {
	...
	public void GcdPKTest() {
		 ...
		 *  Challenge: 	c = random
		 *  Response: 	s1 = Z + c.z	s2 = Q + c.q	s3 = zA + qB + c
		 *  
		...
		BigInteger s3 = adz.multiply(caz).mod(mod).multiply(bdq).mod(mod).multiply(cbq).mod(mod);*/
			...
			BigInteger zad = z.multiply(ad).mod(mod);
			BigInteger qbd = q.multiply(bd).mod(mod);
		BigInteger s3 = zad.add(qbd).mod(mod).add(c).mod(mod);
		System.out.println("s3: " + s3 );
