...
public class PDHReceiver extends PDHParty {
	...
	public BigInteger getStep2b( BigInteger m ) throws TerminateProtocolException{
		...
		checkValues();
		DHKey = Crypto.divide(m, idP.modPow(r, G.getQ()), G.getQ()).modPow(x, G.getQ());
		...
		BigInteger k = Crypto.multiplyList(hashCheckList).modPow(PDHConfig.AUTHENTICATION_SALT_A, G.getQ());
	...
	public void getStepFinal( BigInteger k2 ) throws TerminateProtocolException{
		...
		hashCheckList.add(DHKey);
		hashCheckList.add(password);
		BigInteger k2_expected = Crypto.multiplyList(hashCheckList).modPow(PDHConfig.AUTHENTICATION_SALT_B, G.getQ());
		...
		hashCheckList.add(DHKey);
		hashCheckList.add(password);
		sessionKey = Crypto.hash(Crypto.multiplyList(hashCheckList).modPow(PDHConfig.EXCHANGE_SALT, G.getQ()));
	}
