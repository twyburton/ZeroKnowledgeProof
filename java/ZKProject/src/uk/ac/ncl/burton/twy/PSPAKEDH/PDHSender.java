...
public class PDHSender extends PDHParty {
	...
	public BigInteger getStep3( BigInteger B, BigInteger k) throws TerminateProtocolException{
		...
		BigInteger k_expected = Crypto.multiplyList(hashCheckList).modPow(PDHConfig.AUTHENTICATION_SALT_A, G.getQ());
		...
		hashCheckList.add(DHKey);
		hashCheckList.add(password);
		BigInteger k2_pre = Crypto.multiplyList(hashCheckList).modPow(PDHConfig.AUTHENTICATION_SALT_B, G.getQ());
